...
class ScriptLightningASR:
    def __init__(self,
                 filepath: List[str],
                 device: Optional[str] = "cpu"):
        super(ScriptLightningASR, self).__init__()
        ...
        self.device = device
        self.encoder, self.decoder = self._load_checkpoint(filepath)


    def _load_checkpoint(self, filepath, num_inter_threads=1, num_intra_threads=1):
        encoder = torch.jit.load(filepath.encoder)
        decoder = torch.jit.load(filepath.decoder)
        return encoder, decoder
    ...
    def stream(self,
               ...
               left_context_key_states: torch.Tensor,
               left_context_value_states: torch.Tensor,
               update_length_states: torch.Tensor):
        xs, x_lens = extract_filterbank(torch.cat(speeches).to(self.device), sample_rate, self.device)
        (enc_outs,
         enc_lens,
         memory_states,
         ...
         update_length_states) = self.encoder(xs,
                                              x_lens,
                                              memory_states,
                                              left_context_key_states,
                                              left_context_value_states,
