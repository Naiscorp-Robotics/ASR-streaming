...
def main():
    ...
    # Cấu hình CUDA để tối ưu memory usage
    if torch.cuda.is_available():
        logger.info("Configuring CUDA memory settings...")
        # Giải phóng bộ nhớ cache để đảm bảo bắt đầu với bộ nhớ trống
        torch.cuda.empty_cache()
        
        # Thiết lập memory allocator
        # Các giá trị được đề xuất cho CUDA memory allocation
        if not os.environ.get("PYTORCH_CUDA_ALLOC_CONF"):
            os.environ["PYTORCH_CUDA_ALLOC_CONF"] = "max_split_size_mb:128"
            logger.info("Set PYTORCH_CUDA_ALLOC_CONF=max_split_size_mb:128")
            
        # Thiết lập thiết bị CUDA mặc định
        torch.cuda.set_device(0)
        logger.info(f"Set CUDA device to 0: {torch.cuda.get_device_name(0)}")
        
        # Ghi log thông tin bộ nhớ
        free_mem = torch.cuda.get_device_properties(0).total_memory - torch.cuda.memory_allocated(0)
        logger.info(f"Available GPU memory: {free_mem / 1024**2:.2f} MB")
    else:
